import { computeEclipseStatus, ObserverLocation, OverlapState } from '../src/computeEclipseStatus.js';
...
test('verify eclipse on path of 2024-04-08 event', () => {
  ...
  const observerLocation: ObserverLocation = { lat: lat, lon: lon, elevation: 200 };
  ...
  while (current <= end) {
    ...
    if (result.eclipse != OverlapState.NO_OVERLAP) anyOverlap = true;
